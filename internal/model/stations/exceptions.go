...
var (
	seasonStart    = time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	seasonEnd      = time.Date(2026, 12, 31, 0, 0, 0, 0, time.UTC)
	InSummerSeason = func(t time.Time) bool {
		return t.After(seasonStart) && t.Before(seasonEnd)
	}
	OutsideSummerSeason = func(t time.Time) bool {
		return !InSummerSeason(t)
	}
)
